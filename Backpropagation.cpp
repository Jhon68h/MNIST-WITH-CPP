...
vector<vector<float>> Backpropagation::derivateCostVsSoftmax(
    ...
    */
    ...
    return std::move(derivateCost);//DIMENSIONES IGUAL 60000, 10
...
vector<vector<float>> Backpropagation::hiddenBackPropagation(vector<vector<float>> output,
    ...
    auto row = output.size();
    auto col = delta[0].size();

    vector<vector<float>> gradient(row, vector<float>(col, 0.0f));
    
    auto gradient_size = gradient.size();

    for (size_t i = 0; i < row; i++) {
        for (size_t j = 0; j< col; j++) {
            for(size_t k = 0; k < gradient_size; k++){
                gradient[i][j] = output[i][k] * delta[k][j];
            }
        }
